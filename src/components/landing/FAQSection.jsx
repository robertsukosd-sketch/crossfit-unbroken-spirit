...
export default function FAQSection({ onBookSession }) {
  ...
  useEffect(() => {
    const handleOpenFaq = () => {
      ...
      if (openFaqId) {
        // Find which category contains this FAQ
  ...
  const getFAQs = () => {
    if (language === 'ro') {
      return [
        ...
        { id: 'how-to-start', q: 'Cum pot începe?', a: 'Cel mai simplu mod este să:\n* ', aHowToStart: true },
        ...
        { id: 'scores', q: 'De ce notăm scorurile?', a: 'Folosim ', aScores: true },
      ];
    }
    return [
      ...
      { id: 'how-to-start', q: 'How do I get started?', a: 'The easiest way is to:\n* ', aHowToStart: true },
      ...
      { id: 'scores', q: 'Why do we track scores?', a: 'We use ', aScores: true },
    ];
  };
  ...
  const renderAnswer = (faq) => {
    ...
    return (
      <p className="text-gray-300 text-sm sm:text-base leading-relaxed whitespace-pre-line">
        ...
        {faq.aClassDuration && (
          <button onClick={(e) => { e.stopPropagation(); sessionStorage.setItem('openFaqId', 'faq-wod-meaning'); window.dispatchEvent(new Event('openFaqEvent')); }} className="text-blue-400 hover:text-blue-300 underline transition-colors font-semibold">
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-bold transition-all duration-200 border',
                  isActive
                  ...
                  {openId === faq.id && (
                    ...
                    >
                      <div className="px-5 sm:px-6 py-4 bg-zinc-900/30 border-l-2 border-blue-500 mt-1 rounded-lg space-y-3">
                        {renderAnswer(faq)}
                        {faq.aParking && (
                          <img src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/69948c0d6b8aa61f49f0a23d/4054d6144_image.png" alt={language === 'ro' ? 'Hartă traseu spre sală' : 'Map route to the gym'} className="w-full rounded-xl border border-zinc-700 mt-3" />
                        )}
                        {faq.aImage && (
                          <img src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/69948c0d6b8aa61f49f0a23d/c02900e79_image.png" alt={language === 'ro' ? 'CrossFit - Fitness în 100 de cuvinte' : 'CrossFit - Fitness in 100 words'} className="w-full rounded-xl border border-zinc-700 mt-3" />
                        )}
                      </div>
                    </motion.div>
                  )}
